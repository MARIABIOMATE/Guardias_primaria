...
async function cargarExcel() {
  try {
    ...
    console.log("✅ Archivo Excel cargado correctamente desde Google Sheets.");
    alert("✅ Archivo Excel cargado correctamente desde Google Sheets.");

  } catch (error) {
    console.error("⚠️ Error al cargar el archivo Excel:", error);
    alert("⚠️ Error al cargar el archivo Excel. Verifica la URL o permisos de Google Drive.");
  }
}
...
function inicializarAusencia() {
  ...
  workbookData["HORARIO"].forEach(row => {
    ["Profesor de referencia", "Profesor de apoyo 1", "Profesor de apoyo 2"]
      .forEach(k => { if (row[k]) profesSet.add(row[k]); });
  });
...
function generarCoberturas(listaClases) {
  ...
  fechasUnicas.forEach(fecha => {
    ...
    profesoresAusentes.forEach(pa => {
      const trSub = document.createElement("tr");
      trSub.innerHTML = `<td colspan="4" style="background:#f0f8ff;font-weight:bold;text-align:left">Profesor ausente: ${pa}</td>`;
      tbody.appendChild(trSub);

      const clasesDeEste = clasesDelDia.filter(c=>c.profesorAusente===pa);
      clasesDeEste.forEach(item => {
        ...
        if (!profesorCubre){
          ...
          if (apoyos.length>0){
            const libre = apoyos.sort((a,b)=>{
              const pa=a["Profesor de apoyo 1"], pb=b["Profesor de apoyo 1"];
              const ca=resumen[pa]?.guardias||0, cb=resumen[pb]?.guardias||0;
              return ca-cb;
            })[0];
            profesorCubre = libre["Profesor de apoyo 1"];
          }
        }
...
// === EXPORTAR A EXCEL ===
async function exportarCuadrante(resumen) {
  const wsCuadrante = XLSX.utils.table_to_sheet(document.getElementById("resultado"));
  const datosResumen = [["Profesor", "Guardias acumuladas", "Horas acumuladas"]];
  Object.keys(resumen).forEach(p => {
    datosResumen.push([p, resumen[p].guardias, resumen[p].horas]);
  });
